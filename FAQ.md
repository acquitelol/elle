...
| Compiler backend  | QBE             | LLVM is too bloated for the language's needs, therefore QBE is used instead                 |
...
| Loops             | Standard        | `while cond {}` or `for x := 1; x < 10; x += 1` or `for x in [1, 2, 3]` or `for x in 0..10` |
